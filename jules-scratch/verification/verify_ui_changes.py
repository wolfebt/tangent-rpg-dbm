...
def run_verification(playwright):
    ...
    page.screenshot(path='jules-scratch/verification/rules_codex_verification.png')
    ...
    # Take a screenshot of the Species modal
    page.screenshot(path='jules-scratch/verification/species_modal_verification.png')

    # Verify blue border styling
    # The screenshots above should show the blue borders on the elements.
    # We can also explicitly check the border color of an element.
    modal_content = page.locator('.modal-content').first
    border_color = modal_content.evaluate('(element) => window.getComputedStyle(element).borderColor')

    print(f"Modal border color: {border_color}")
